...
def process_image(img, yolo8, face_det, anti_spoof, iou_thresh, score_thresh):
    ...
    try:
        ...
        # Diccionario de dispositivos que pueden generar spoofing
        SPOOFABLE_CLASSES = {
            k: v for k, v in classNames.items()
            if v in ["cell phone", "tv", "laptop", "tablet", "monitor", "book"]  # puedes añadir más
        }
        
        # Convertir a set para búsquedas rápidas
        SPOOFABLE_IDS = set(SPOOFABLE_CLASSES.keys())
        ...
        # Bounding boxes
        faces_boxes = []
        spoof_objects = []  # objetos sospechosos de spoof
        ...
        for r in results:
            for box in r.boxes:
                ...
                elif cls in SPOOFABLE_IDS:
                    spoof_objects.append({
                        "box": np.array([x1, y1, x2, y2]),
                        "type": classNames[cls]
                    })
        
        logger.info("Detecciones: Rostros=%d, Objetos sospechosos=%d", len(faces_boxes), len(spoof_objects))
        
        # Detección de spoofing
        for f in faces_boxes:
            for obj in spoof_objects:
                iou_val = iou(f, obj["box"])
                if iou_val > iou_thresh:
                    logger.warning("IoU alto entre rostro y objeto (%s): %.2f", obj["type"], iou_val)
                    return {
                        "result": "FAKE",
                        "reason": f"IoU alto entre rostro y {obj['type']}"
                    }
        start = time.time()
