...
SUSPECT_CLASSES = ["cell phone", "tv", "laptop", "tablet", "monitor", "book"]
PERSON_CLASS = "person"
...
def increased_crop(img,bbox,bbox_inc=1.6):
    h,w=img.shape[:2]
...
def main():
    ...
    SUSPECT_IDS = [k for k, v in classNames.items() if v in SUSPECT_CLASSES]
    PERSON_ID = [k for k, v in classNames.items() if v == PERSON_CLASS][0]
    ...
    for img_path in sorted(in_dir.glob("*")):
        ...
        for r in results:
            for b in r.boxes:
                ...
                elif label in SUSPECT_CLASSES:
                    spoof_boxes.append((label, np.array([x1, y1, x2, y2])))

                # dibuja (opcional)  
                color = (200, 0, 200)  # Color por defecto
        ...
        for f in faces:
            if spoof: break
            for obj_name,p in spoof_boxes:
                iou_val = iou(f, p)
                if iou_val > args.iou:
                    spoof=True
                    mensaje = f"❌ SPOOF {obj_name} (IoU={iou_val:.2f})"
                    cv2.putText(img, mensaje, (f[0], f[1]-25), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 3)                    
                    print(f"⚠️  SPOOF DETECTADO: rostro superpuesto con {obj_name} (IoU={iou_val:.2f})")
                    break
